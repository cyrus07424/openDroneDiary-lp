...
export async function POST(request: NextRequest) {
  try {
    ...
    const success = await slackService.notifyDataRegistration(userInfo, dataType, {
      ...additionalData,
      dataInfo,
    ...
    if (success) {
      return NextResponse.json(
        { message: 'Data registration notification sent successfully' },
        { status: 200 }
      );
    } else {
      return NextResponse.json(
    ...
    console.error('Error in data registration notification:', error);
    return NextResponse.json(
