...
export async function POST(request: NextRequest) {
  try {
    ...
    const success = await slackService.notifyUserLogin(userInfo, {
      ...additionalData,
    ...
    if (success) {
      return NextResponse.json(
        { message: 'User login notification sent successfully' },
        { status: 200 }
      );
    } else {
      return NextResponse.json(
    ...
    console.error('Error in user login notification:', error);
    return NextResponse.json(
